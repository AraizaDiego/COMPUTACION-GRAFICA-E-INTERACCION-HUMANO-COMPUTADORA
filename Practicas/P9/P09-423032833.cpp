...
int rotdragon = 0;
float posdragonxz = 0.0f;*/
float cambia = 0.0f;
float toffsetvelocidadnum = 0.0f;
bool num2cambia = true;
float Tpuerta = 0.0f;
float Rpuerta = 0.0f;
...
int main()
{
	...
	DragonAI_M = Model();
	DragonAI_M.LoadModel("Models/DragonAI.obj");
	DragonAD_M = Model();
	DragonAD_M.LoadModel("Models/DragonAD.obj");
	EstructuraP_M = Model();
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		*/
		...
		//animacion cambio numero
		if (cambia >= 1.0f) {
			toffsetvelocidadnum = 1.0f;
			cambia = 0.0f;
		}
		else {
			toffsetvelocidadnum = 0.0f;
			cambia += 0.2 * deltaTime;
		}
		*/
		...
		//Ala izquierda
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.544f*posdragonxz, 1.071f, 0.145f * posdragonxz));
		model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		model = glm::rotate(model, rotdragon * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(rotalas), glm::vec3(-0.8910f, 0.4540f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		DragonAI_M.RenderModel();

		//Ala derecha
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.544f * posdragonxz, 1.071f, -0.033 * posdragonxz));
		model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		model = glm::rotate(model, rotdragon * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(rotalas), glm::vec3(0.8910f, -0.4540f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		DragonAD_M.RenderModel(); 
		*/
		...
		*/
		/*
		toffsetnumerocambiau += 0.25 * toffsetvelocidadnum;
		...
		color = glm::vec3(1.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		if (toffsetvelocidadnum >= 1) {
			num2cambia = !num2cambia;
		}
